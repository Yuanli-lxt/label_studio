...
PROBE_FILE="${1:-demo_data/tasks/image_classification_regression_probes.json}"
...
label_config = (
    ...
    '<Choices name="image_label" toName="image">'
    '<Choice value="Product"/><Choice value="Other"/>'
    '</Choices></View>'
)
...
for pred in pred_payload.get("results", []):
    ...
    task_id = pred_id.replace("_cls", "") if pred_id else None
    if task_id:
        preds_by_id[task_id] = pred

print("Image Classification Regression Probes")
...
for probe in probes:
    ...
    expected_source = probe.get("expected_prediction_source")
    ...
    if expected_source is not None and source != expected_source:
        reasons.append(f"expected_source={expected_source}, got={source}")
    ...
    print(
        ...
        f"pred={predicted_label} score={score} uncertain={uncertain} source={source} | "
...
print("All image regression probes passed.")
PY
