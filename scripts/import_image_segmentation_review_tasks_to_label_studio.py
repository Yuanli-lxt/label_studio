#!/usr/bin/env python3
"""Import image-segmentation review tasks into a Label Studio project, with dedupe."""
...
DEFAULT_SEGMENTATION_TASKS_PATH = Path("demo_data/tasks/image_segmentation_review_tasks.json")
...
def load_segmentation_review_tasks(path: Path) -> List[Dict[str, Any]]:
    if not path.exists():
        raise FileNotFoundError(
            f"review tasks file not found: {path}. Restore demo_data/tasks/image_segmentation_review_tasks.json"
        )
    try:
...
def resolve_project_id(
    ...
    title = (project_title or DEFAULT_SEGMENTATION_PROJECT_TITLE).strip()
    project = client.find_project_by_title(title)
    if not project:
        raise RuntimeError(
            f"Label Studio project not found by title {title!r}. "
            "Run scripts/bootstrap_label_studio_image_segmentation_review.py first."
        )
    return int(project["id"])


def import_segmentation_review_tasks(
    client: LabelStudioClient,
    tasks_path: Path,
    ...
    tasks = load_segmentation_review_tasks(tasks_path)
...
def main(argv: list[str] | None = None) -> int:
    ...
    try:
        ...
        tasks_path = args.tasks_path or Path(
            os.getenv("IMAGE_SEG_REVIEW_TASKS_PATH", str(DEFAULT_SEGMENTATION_TASKS_PATH))
        )
        project_title = args.project_title or os.getenv(
        ...
        info(f"project selector: id={args.project_id} title={project_title}")
    ...
    ok("Image segmentation review task import summary")
